...
class LinearElement:
    type: LineType
    _nodes: list[Position]

    def __init__(self, type: LineType, nodes: list[Position] = None) -> None:
    ...
    def add_preview_node(self) -> None:
        ...
        self._preview_node = Position(0, 0)

    def discard_preview_node(self) -> None:
        """Discard the preview node."""
        self._preview_node = None

    def move_preview_node_to(self, node_pos: Position) -> None:
        ...
        match(num_nodes):
            ...
            case 1:
                if (self._nodes[0].x == node_pos.x or self._nodes[0].y == node_pos.y):
                    ...
                    self._intermediate_node = Position(node_pos.x, self._nodes[0].y)
                    self._preview_node = node_pos
            case _:
                ...
                if (self._nodes[-1].x == node_pos.x or self._nodes[-1].y == node_pos.y):
                    ...
                    self._intermediate_node = Position(node_pos.x, self._nodes[-1].y)
                    self._preview_node = node_pos
